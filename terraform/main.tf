...
resource "null_resource" "deploy-function-app" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      az functionapp deployment source config-zip -g ${azurerm_resource_group.sub-vending-rsg-001.name} -n ${azurerm_function_app.sub-vending-func-app-001.name} --src ..\zipdeploy\Subscription-Vending-Fucntion-v1-0-1.zip
